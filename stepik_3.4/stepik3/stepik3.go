...
func main() {
	...
	in := bufio.NewScanner(os.Stdin) // читаем из консоли ввода

	num := 0
	for in.Scan() { // возвращает true, пока файл не будет прочитан до конца
		if len(in.Text()) == 0 {
			break
		}
		n, err := strconv.Atoi(in.Text()) // строку в число
		if err != nil {
			panic(err)
		}
		num += n
